        ...
        for rect in rects:
            ...
            if ear <  EYE_AR_THRESH:
                ...
                if counter >= EYE_AR_CONSEC_FRAMES:
                    if sleep_flag < 0:
                        cv2.putText(frame, "Sleep Detected", (150, 150),cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                        sleep_flag = 1
                        total += 1
                    #     write_read('1')
            ...
            cv2.putText(frame, "MAR: {:.2f}".format(mouthEAR), (540, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            if total+total_yawn == 2:

                winsound.Beep(2000,1000)
                total+=1

            elif total+total_yawn == 3:
                winsound.Beep(2000,1000)
                total+=1
                
            elif total+total_yawn == 4:
                winsound.Beep(2000,1000)
                total+=1
                
            elif total+total_yawn > 5:
                engine.say("You are sleeping, It could be dangerous. I'm Alerting Your Emergency Contact Regarding this.")
                write_read('1')
                print("stopping machine ")
                engine.runAndWait()
                total = 0
